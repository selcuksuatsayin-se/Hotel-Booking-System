...
namespace HotelSearchService.Controllers
{
    ...
    public class HotelsController : ControllerBase
    {
        private readonly AppDbContext _context;
        private readonly IMemoryCache _cache;
        private readonly ILogger<HotelsController> _logger; // <--- 1. LOGGER EKLENDİ

        public HotelsController(AppDbContext context, IMemoryCache cache, ILogger<HotelsController> logger)
        {
            _context = context;
            _cache = cache;
            _logger = logger; // <--- 1. LOGGER BAĞLANDI
        }
        ...
        public async Task<ActionResult<IEnumerable<HotelReadDto>>> GetHotels()
        {
            ...
            if (!_cache.TryGetValue(cacheKey, out List<HotelReadDto> hotels))
            {
                // --- CACHE MISS (Bulunamadı) ---
                // Terminalde sarı/beyaz renkte uyarı verecek
                _logger.LogWarning("--> Cache MISS. The search is made in the database");
            ...
            {
                // --- CACHE HIT (Bulundu) ---
                // Terminalde bilgi mesajı verecek
                _logger.LogInformation("--> Cache HIT. Results fetched from memory.");
            }
        ...
        public async Task<ActionResult<HotelReadDto>> GetHotel(
            ...
            [FromQuery] DateOnly? checkOut)
        {
            // Detay sayfasında cache kullanmıyoruz (Fiyatlar dinamik değiştiği için)
            // Ama buraya da log atabiliriz istiyorsan.
            ...
            foreach (var room in hotel.Rooms)
            {
                decimal effectivePrice = room.BasePrice;
